 ...
 * live output level (0–1) from the `volume-level` event — feed it to a sphere or
 * orb to make it react to the voice in real time.
 *
 * The Vapi instance (and Daily's WebRTC engine) is created lazily on the first
 * `start()` call, not on mount, so mounting this globally (e.g. a floating
 * widget) doesn't pull the SDK onto every page until the user actually calls.
 */
